...
if not st.session_state.get("data_ready", False):
    ...
    st.stop()
df_cat = st.session_state["total_scorecard_cat"]
...
df_cat_styled = df_cat.copy()
df_cat_styled.iloc[[1,2]] = df_cat_styled.iloc[[1,2]].applymap(lambda x: f"{x:.0f}")
df_cat_styled.iloc[[0,3]] = df_cat_styled.iloc[[0,3]].applymap(lambda x: f"{x:.2f}")
df_cat_styled.iloc[[4]] = df_cat_styled.iloc[[4]].applymap(lambda x: f"{x:.2%}")

df_cat_styled = backend.style_df(df_cat_styled,colo1="orange", color2="lawngreen")

st.dataframe(df_cat_styled, use_container_width=True)

sorted = st.checkbox("Ordenar")

col1, col2, col3 = st.columns([1/3, 1/3, 1/3])
with col1:
    df_to_plot = df_cat.loc["Rotación"]
    if sorted:
...
with col2:
    df_to_plot = df_cat.loc["Días de inventario - 250 días al año"]
    if sorted:
        df_to_plot = df_to_plot.sort_values(ascending=False)
    total = df_to_plot["Costo Totales"]
    df_to_plot.drop("Costo Totales", inplace=True)
...
with col3:
    df_to_plot = df_cat.loc["Días de inventario - 360 días al año"]
    if sorted:
        df_to_plot = df_to_plot.sort_values(ascending=False)
    total = df_to_plot["Costo Totales"]
    df_to_plot.drop("Costo Totales", inplace=True)
...
col1, col2, col3 = st.columns([1/3, 1/3, 1/3])
with col1:
    df_to_plot = df_cat.loc["Meses de inventario"]
    if sorted:
        df_to_plot = df_to_plot.sort_values(ascending=False)
    total = df_to_plot["Costo Totales"]
    df_to_plot.drop("Costo Totales", inplace=True)
...
with col2:
    df_to_plot = df_cat.loc["GMROI - Gross Margin Return on Inventory"]
    if sorted:
        df_to_plot = df_to_plot.sort_values(ascending=False)
    total = df_to_plot["Costo Totales"]
    df_to_plot.drop("Costo Totales", inplace=True)
