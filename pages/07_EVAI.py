...
def main():
    ...
    if st.session_state["authentication_status"] is False:
        ...
        df_transformed = backend.merge_icrs(df_transformed, ICRs)
        ...
        with tab_1:
            ...
            with tab_1:
                ...
                with todos:
                    ...
                    df_filtered_sorted = df_filtered.sort_values(by=['EVAI (valor agregado del inventario)'], ascending=False)
                    # Muestra el reporte de productos estilizado
                    backend.show_df_evai(df_filtered_sorted)
                    # Crear un gráfico de barras para mostrar el EVAI por subcategoría
                    backend.barchart_evai(df_filtered_sorted, subcat)

                # En la subpestaña "Positivos", mostrar el DataFrame de productos positivos
                with positivos:
                    # Filtra por los productos que tengan EVAI positivo
                    df_filtered_pos = df_filtered[df_filtered["EVAI (valor agregado del inventario)"]>=0]
                    # Ordena de mayor a menor en base al EVAI
                    df_filtered_pos = df_filtered_pos.sort_values(by=['EVAI (valor agregado del inventario)'], ascending=False)
                    # Muestra el reporte de productos estilizado
                    backend.show_df_evai(df_filtered_pos)
                    # Crear un gráfico de barras para mostrar el EVAI por subcategoría
                    backend.barchart_evai(df_filtered_pos, subcat)

                # En la subpestaña "Negativos", mostrar el DataFrame de productos negativos 
                with negativos:
                    # Filtra por los productos que tengan EVAI negativo
                    df_filtered_neg = df_filtered[df_filtered["EVAI (valor agregado del inventario)"]<0]
                    # Ordena de menor a mayor en base al EVAI
                    df_filtered_neg = df_filtered_neg.sort_values(by=['EVAI (valor agregado del inventario)'], ascending=True)
                    # Muestra el reporte de productos estilizado
                    
                    backend.show_df_evai(df_filtered_neg)
                    # Crear un gráfico de barras para mostrar el EVAI por subcategoría
                    backend.barchart_evai(df_filtered_neg, subcat)
