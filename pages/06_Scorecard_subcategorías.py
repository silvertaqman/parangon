...
def main():
    ...
    if st.session_state["authentication_status"] is False:
        ...
        value_produc_table= backend.pivot_value_table(df_transformed, 'subcat_producto')
        ...
        def scorecard(percent_produc_table,table_name, 
                    inver_param_names, cost_param_names, 
                    inver_param_values, cost_param_values,
                    capital_cost_value, drivers_key, 
                    ):
            ...
            scorecard = backend.show_scorecard(percent_produc_table,
                                            ...
                                            inver_param_names = inver_param_names,
                                            cost_param_names = cost_param_names,
                                            drivers_list = drivers["subcategorias"][drivers_key],
                                            df_scorecard_all=df_scorecard_all)

            # Definir una función para guardar los cambios realizados en el scorecard.
            def save():
                drivers["subcategorias"][drivers_key] = scorecard["Drivers"].to_list()[:-1]
            ...
            if scorecard["Drivers"].to_list()[:-1] != drivers["subcategorias"][drivers_key]:
        ...
        with st.expander("Costos del almacén"):
            ...
            with tab_1:
                alm_scorecard = scorecard(percent_produc_table.drop("Total", axis=1), 
                    table_name = "Costos del almacén",
                    inver_param_names = param_names["almacenaje"]["inversiones"],
                    cost_param_names = param_names["almacenaje"]["costos_gastos"],
                    inver_param_values = parameters["alm_inversiones"],
                    cost_param_values = parameters["alm_costosgastos"],
                    capital_cost_value = parameters["gen_financieros"][0],
                    drivers_key = "alm_drivers")
            with tab_2:
                backend.download_dataframe(alm_scorecard, name="scorecard_cost_almacen_subcat")
        ...
        with st.expander("Costos del inventario"):
            ...
            with tab_2:
                backend.download_dataframe(inv_scorecard, name="scorecard_cost_invent_subcat")       
        ...
        with st.expander("Costo total y KPI's"):
            ...
            with tab_1:
                ...
                st.session_state["total_scorecard_subcat"] = total_cost_df
            with tab_2:
                backend.download_dataframe(total_cost_df, name="scorecard_cost_total_subcat")
        
        st.session_state['total_sub'] = total_cost_df
