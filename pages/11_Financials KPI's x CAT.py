...
df_cat = df_cat.loc[["Costo total de mantener inventario (ICC)",
...
df_cat_styled.iloc[[1,2]] = df_cat_styled.iloc[[1,2]].applymap(lambda x: f"{x:.2%}")
...
with col1:
    df_to_plot = df_cat.loc["Costo total de mantener inventario (ICC)"]
    if sorted:
...
with col2:
    df_to_plot = df_cat.loc["Tasa de mantener el inventario (ICR)"]
    if sorted:
        df_to_plot = df_to_plot.sort_values(ascending=False)
    total = df_to_plot["Costo Totales"]
    df_to_plot.drop("Costo Totales", inplace=True)
...
with col3:
    df_to_plot = df_cat.loc["Costo de mantener inventarios/ventas"]
    if sorted:
        df_to_plot = df_to_plot.sort_values(ascending=False)
    total = df_to_plot["Costo Totales"]
    df_to_plot.drop("Costo Totales", inplace=True)
...
col1, col2, col3 = st.columns([1/3, 1/3, 1/3])
with col1:
    df_to_plot = df_cat.loc["EVAI - Valor agregado del inventario"]
    if sorted:
        df_to_plot = df_to_plot.sort_values(ascending=False)
    total = df_to_plot["Costo Totales"]
    df_to_plot.drop("Costo Totales", inplace=True)
