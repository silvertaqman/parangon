...
def main():
    ...
    if st.session_state["authentication_status"] is False:
        ...
        def show_viz(df):
            ...
            with col1:
                backend.barchart_all_products(df)
            ...
            with col2:
                backend.piechart_all_products(df)
        ...
        with tab_1:
            ...
            with todos:
                df_all = backend.show_df_repevai(df_transformed, ICRs)
                show_viz(df_all)

            # En la subpestaña "Positivos", mostrar el DataFrame de productos positivos
            with positivos:
                df_positivos = backend.show_df_repevai(df_transformed, ICRs, range="positive")
                show_viz(df_positivos)

            # En la subpestaña "Negativos", mostrar el DataFrame de productos negativos 
            with negativos:
                df_negativos = backend.show_df_repevai(df_transformed, ICRs, range="negative")
